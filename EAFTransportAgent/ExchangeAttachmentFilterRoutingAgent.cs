...
namespace MrPear.Net.ExchangeAttachmentFilter
{
    public class ExchangeAttachmentFilterRoutingAgent : RoutingAgent
    {
        ...
        {
            // whitelisted
            if (_exchangeAttachmentFilterConfig.AttachmentsWhitelist.Any(
                f => Regex.IsMatch(fileName, WildcardToRegex(f))))
                return new AttachmentFilterStatus(AttachmentFilterStatusEnum.Accept, "Filename WHITELIST");

            // remove attachment
            if (_exchangeAttachmentFilterConfig.AttachmentsRemove.Any(
                f => Regex.IsMatch(fileName, WildcardToRegex(f))))
                return new AttachmentFilterStatus(AttachmentFilterStatusEnum.RemoveAttachment, "Filename REMOVE");

            // reject attachment
            if (_exchangeAttachmentFilterConfig.AttachmentsReject.Any(
                f => Regex.IsMatch(fileName, WildcardToRegex(f))))
                return new AttachmentFilterStatus(AttachmentFilterStatusEnum.RejectMessage, "Filename REJECT");

            return new AttachmentFilterStatus(AttachmentFilterStatusEnum.Accept, "No match");
        }
            ...
            try
            {
                using (var zipFile = new ZipFile(openXmlDocumentStream))
                {
                    ...
                    if (contentTypesEntry == null)
                        return new AttachmentFilterStatus(AttachmentFilterStatusEnum.Accept, "OpenXml \"[Content_Types].xml\" not found");
                    ...
                    if (rootElement == null)
                        return new AttachmentFilterStatus(AttachmentFilterStatusEnum.Accept, "OpenXml root element \"Types\" not found.");
                    ...
                    return type == null
                        ...
                        : new AttachmentFilterStatus(AttachmentFilterStatusEnum.RejectMessage, "OpenXml macroEnabled");
                }
            }
            catch (ZipException ex)
            {
                // does not look like zip
                return new AttachmentFilterStatus(AttachmentFilterStatusEnum.Accept,
                    $"OpenXml ERROR [{ex.GetType()}]: {ex.Message}");
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Checks if the filename is indicating supported archive (by file extension).
        /// </summary>
        /// <param name="fileName">File name to check.</param>
        /// <returns>Returns true if the filename is archive.</returns>
        public static bool IsArchive(string fileName)
        {
            return Config.ArchiveFileTypes.Any(
                f => Regex.IsMatch(fileName, WildcardToRegex(f)));
        }

        public static bool IsOpenXmlDocument(string fileName)
        {
            return Config.OpenXmlDocumentFileTypes.Any(
                f => Regex.IsMatch(fileName, WildcardToRegex(f)));
        }
